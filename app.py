...
with st.sidebar:
    ...
    # 修正下載：確保下載的是 df_filtered
    if not df_filtered.empty:
        csv = df_filtered.to_csv(index=False).encode('utf-8-sig')
        st.download_button(label="📥 Download current data (.csv)", data=csv, file_name='spotify_analysis_export.csv', mime='text/csv')
...
with tab1:
    ...
    st.plotly_chart(fig1, width='stretch') # ✅ 修正語法
    ...
    if len(art_stats) > 1:
        ...
        st.write(f"📊 **Stat Correlation:** `{corr_val:.2f}` (Followers vs. Chart Success)")
